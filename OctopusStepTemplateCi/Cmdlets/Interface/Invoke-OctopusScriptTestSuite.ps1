...
function Invoke-OctopusScriptTestSuite
{
    ...
    foreach( $fileToProcess in $filesToProcess )
    {
        ...
        $baseResultsFile = Join-Path $ResultFilesPath $fileToProcess.Name.Replace(".ps1", ".TestResults.xml");

        $testScriptFile  = $fileToProcess.FullName.Replace(".ps1", ".Tests.ps1");
        $testResultsFile = $baseResultsFile;
        $testResults = Invoke-PesterForTeamCity -TestName        $fileToProcess.Name `
        ...
        $testResultsFile = Join-Path $ResultFilesPath $fileToProcess.Name.Replace(".ps1", ".generic.TestResults.xml");
        $testResults = Invoke-PesterForTeamCity -TestName        $fileToProcess.Name `
        ...
        if( $fileToProcess.Name -like $ScriptModuleFilter )
        {
            $testScriptInfo = @(
            ...
            $testResultsFile = Join-Path $ResultFilesPath $fileToProcess.Name.Replace(".ps1", ".script-module.TestResults.xml");
            $testResults = Invoke-PesterForTeamCity -TestName        $fileToProcess.Name `
                                                    -Script          $testScriptInfo `
                                                    -TestResultsFile $testResultsFile `
                                                    -SuppressPesterOutput:$SuppressPesterOutput;
        ...
        elseif( $fileToProcess.Name -like $StepTemplateFilter )
        {
            $testScriptInfo = @(
            ...
            $testResultsFile = Join-Path $ResultFilesPath $fileToProcess.Name.Replace(".ps1", ".step-template.TestResults.xml");
            $testResults = Invoke-PesterForTeamCity -TestName        $fileToProcess.Name `
                                                    -Script          $testScriptInfo `
                                                    -TestResultsFile $testResultsFile `
                                                    -SuppressPesterOutput:$SuppressPesterOutput;
